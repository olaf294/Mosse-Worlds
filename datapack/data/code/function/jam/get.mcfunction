...
data modify storage api jam set from storage api jam_world.response.jam

execute unless data storage api jam_world.response.jam.id run return run function code:jam/init


scoreboard players set .jam_id misc -1
execute store result score .jam_id misc run data get storage api jam_world.response.jam.id
...
execute positioned -19 64 -5 run function code:jam/display with storage api jam_world.response