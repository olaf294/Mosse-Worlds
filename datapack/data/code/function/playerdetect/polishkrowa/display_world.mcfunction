...
execute as @e[type=text_display,tag=polish_world] run data modify entity @s text.extra[0] set from storage player_detect temp.a.world.response.raw_name
execute as @e[type=text_display,tag=polish_world] if data storage player_detect temp.a{uuid:"lobby"} run data modify entity @s text.extra[0] set value {text:"Lobby",color:gold}
