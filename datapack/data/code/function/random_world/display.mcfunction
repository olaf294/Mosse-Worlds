...
"\n  ",{storage:api,nbt:"random.raw_name",interpret:1b}]

execute store result score .votes misc run data get storage api random.votes
execute store result score .visits misc run data get storage api random.visits

# Calculate Ratio
execute store result score .votes2 misc run data get storage api random.votes
...
$execute unless data storage api {random:{version:"26.2"}} run return run tellraw @a[tag=info,distance=..10] [{text:" - - - - - - - - - - - - - - - - ",color:gray},{text:"\nRᴀɴᴅᴏᴍ Wᴏʀʟᴅ: Cʟɪᴄᴋ ",color:gray},{text:"[HERE]",color:gold,click_event:{action:"run_command",command:"/world $(world_uuid)"},hover_event:{action:"show_text",value:{text:"Wᴏʀʟᴅ UUID: $(world_uuid)",color:green}}},{text:" ᴛᴏ ᴘʟᴀʏ.",color:gray},\
"\n  ",{storage:api,nbt:"random.raw_name",interpret:1b},{text:"\n  Vᴇʀꜱɪᴏɴ: ",color:"#888888"},{storage:api,nbt:"random.version",color:yellow,interpret:1b},"\n  ",\
